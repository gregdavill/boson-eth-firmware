...
int microudp_send_2(unsigned int length)
{
	...
	txbuffer->frame.contents.udp.udp.src_port = htons(0);
	txbuffer->frame.contents.udp.udp.dst_port = htons(0);
	txbuffer->frame.contents.udp.udp.checksum = 0;
...
void _microudp_start(const unsigned char *macaddr, unsigned int ip)
{
	int i;
...
void eth_print_mac(void){
	printf("%02x:%02x:%02x:%02x:%02x:%02x", cached_mac[0],cached_mac[1],cached_mac[2],cached_mac[3],cached_mac[4],cached_mac[5]);
}
